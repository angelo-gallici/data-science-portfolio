...
def convert_dates(df):
    ...
    df['tramite_anio'] = df['tramite_fecha'].dt.year
    df['tramite_mes'] = df['tramite_fecha'].dt.month
    df['inscripcion_anio'] = df['fecha_inscripcion_inicial'].dt.year
    df['inscripcion_mes'] = df['fecha_inscripcion_inicial'].dt.month
    return df

def corregir_marcas(df, columna):
    marcas = df[columna].str.upper().str.strip()
    correcciones = {
    ...
    df[columna] = marcas.replace(correcciones)
    return df

def corregir_otros(df):
    df['tramite_tipo'] = df['tramite_tipo'].replace({
        'DENUNCIA DE ROBO O HURTO / RETENCION INDEBIDA': 'DENUNCIA DE ROBO O HURTO'
    })
    df['registro_seccional_provincia'] = df['registro_seccional_provincia'].replace({
        'Ciudad Autónoma de Bs.As.': 'Ciudad Autónoma de Buenos Aires'
    })
    df['titular_domicilio_provincia'] = df['titular_domicilio_provincia'].replace({
...
def limpiar_modelos(df):
    marcas = df['automotor_marca_descripcion'].dropna().str.upper().unique().tolist()
